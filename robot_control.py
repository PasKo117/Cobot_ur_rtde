...
class Robot:
    ...
    def init_robot(self):
        self.logger.debug(f"Подключение к роботу (RTDE) IP: {self.IP}...")
        try:
    ...
    def get_speeds(self):
        ...
        if self.joystick.get_button(2) and not self.joystick.get_button(3):
            speeds[2] = 1 * -self.linear_velocity
        if self.joystick.get_button(3) and not self.joystick.get_button(2):
            speeds[2] = 1 * self.linear_velocity
        ...
        speeds[3] = (axis1 if abs(axis1) > self.deadzone else 0) * self.rotational_velocity
        speeds[4] = (axis0 if abs(axis0) > self.deadzone else 0) * self.rotational_velocity
        speeds[5] = (axis3 if abs(axis3) > self.deadzone else 0) * self.rotational_velocity
    ...
    def get_slave_speeds(self):
        ...
        speeds = [
            self.speeds[2] * z1 + self.speeds[0] * xx + self.speeds[1] * yx,
            self.speeds[2] * z2 + self.speeds[0] * xy + self.speeds[1] * yy,
            0, 0, 0, 0
        ]
        return speeds
    ...
    def main_loop(self):
        ...
        while self.running:
            ...
            try:
                if self.joystick.get_button(0):  # зажат курок
                    self.ctrl.speedL(self.speeds, acceleration=0.1, dt=0.008)
                    if self.auto.value:
                        if self.is_master:
            ...
            if self.joystick.get_button(6) and (
            not self.joystick.get_button(7)) and self.auto.value == 0 and self.is_master:
                self.auto.value = 1
                self.logger.info("Система переведена в СИНХРОННЫЙ режим")

            if self.joystick.get_button(7) and (
            not self.joystick.get_button(6)) and self.auto.value == 1 and self.is_master:
                self.auto.value = 0
